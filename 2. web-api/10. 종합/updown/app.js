...
function checkAnswer($numbers, $target){
    ...
    //정답 맞췄을 때는 정답처리를 하는 함수 호출(processCorrect)
    //up 또는 down일 경우에는 min과 max값 변경.
    //begin과 end값도 변경.
    //aside 태그의 애니메이션을 담당할 executeUpDownAnimation호출
    if(secret === answer) {
    ...
    makeNumberIcons();// min, max가 변경되어있기 때문에 그에 맞춰서 아이콘 다시 배치
}
...
(function() {
    ...
    $numbers.addEventListener('click', function(e){
        ...
        console.log(`${e.target.textContent}가 클릭됨!`)//사용자가 선택한 값
